// export const characters = [
//   // Iron Man
//   {
//     name: "Tony Stark",
//     alias: "Iron Man",
//     img1: "https://www.specfictionshop.com/cdn/shop/products/315455127_2253071438203857_6311282012262232749_n_2000x.jpg?v=1669836598",
//     homeworld: "Earth",
//     abilities: [
//       "Genius-level intellect",
//       "Powered armor suit",
//       "Superhuman strength",
//     ],
//     team: "Avengers",
//     description:
//       "Tony Stark, also known as Iron Man, is a brilliant inventor, philanthropist, and industrialist. He uses his genius-level intellect and a highly advanced powered armor suit to fight for justice and protect the world from various threats. Stark is a charismatic playboy turned superhero who combines his technological prowess with unwavering determination to uphold the values of heroism and innovation.",
//   },
//   {
//     name: "Pepper Potts",
//     alias: "Rescue",
//     abilities: ["Powered armor suit", "Advanced technology skills"],
//     team: "Avengers",
//     img1: "https://i.pinimg.com/originals/5e/fe/55/5efe55d2a7785a311511736a1a37bcbb.jpg",
//     homeworld: "Earth",
//     description:
//       "Pepper Potts, the formidable Rescue, is an accomplished businesswoman and technological genius. She dons a powered armor suit equipped with an array of advanced technology and weaponry, allowing her to stand alongside the Avengers in the battle against supervillains and global threats. Pepper is a determined and fearless heroine with an unwavering commitment to justice.",
//   },
//   {
//     name: "James 'Rhodey' Rhodes",
//     alias: "War Machine",
//     abilities: ["Powered armor suit", "Military expertise"],
//     team: "Avengers",
//     img1: "https://i.pinimg.com/474x/ec/3f/4f/ec3f4f8053892365046fc6cd2f7955eb.jpg",
//     homeworld: "Earth",

//     description:
//       "James 'Rhodey' Rhodes, also known as War Machine, is a highly skilled pilot and a military expert. He operates a heavily armed powered armor suit that enhances his strength and combat capabilities. With a strong sense of duty and loyalty to his allies, Rhodey plays a crucial role in the Avengers' efforts to maintain peace and protect the world.",
//   },
//   {
//     name: "Riri Williams",
//     alias: "Ironheart",
//     abilities: ["Powered armor suit", "Genius-level intellect"],
//     team: "Young Avengers",
//     img1: "https://i.pinimg.com/736x/e3/02/9b/e3029b056f1deaf2011d72d0bce0bafa.jpg",
//     homeworld: "Earth",

//     description:
//       "Riri Williams, known as Ironheart, is a prodigious young inventor with a genius-level intellect. She has designed her own advanced powered armor suit, which she uses to fight against threats as a member of the Young Avengers. Riri's intelligence, determination, and innovative spirit make her a rising star in the superhero community.",
//   },

//   // Spider-Man
//   {
//     name: "Miles Morales",
//     alias: "Spider-Man",
//     abilities: ["Bio-electrokinesis", "Wall-crawling"],
//     team: "Champions",
//     img1: "https://w0.peakpx.com/wallpaper/15/497/HD-wallpaper-marvel-spider-man-marvel-spider-man-game-games.jpg",
//     homeworld: "Earth",
//     img3: "url_to_img3_for_Natasha_Stark",
//     description:
//       "Miles Morales, the teenage Spider-Man, has bio-electrokinesis powers, in addition to wall-crawling abilities. As a member of the Champions, Miles embodies the spirit of the Spider-Man legacy, fighting for equality and justice in the Marvel Universe while navigating the challenges of being a young hero.",
//   },

//   {
//     name: "Cindy Moon",
//     alias: "Silk",
//     abilities: ["Web-shooting", "Superhuman agility"],
//     team: "Agents of Atlas",
//     img1: "https://media-cldnry.s-nbcnews.com/image/upload/newscms/2015_53/894026/silk_1_cover.jpg",
//     homeworld: "Earth",
//     description:
//       "Cindy Moon, codenamed Silk, is a skilled hero with the power of web-shooting and remarkable superhuman agility. She is an essential member of the Agents of Atlas, using her arachnid-like abilities to combat threats and safeguard the world.",
//   },

//   {
//     name: "Steve Rogers",
//     alias: "Captain America",
//     abilities: ["Peak human strength", "Vibranium shield"],
//     team: "Avengers",
//     img1: "https://w0.peakpx.com/wallpaper/223/397/HD-wallpaper-captain-america-avengers-endgame-captain-america-marvel-marvel-comics-marvel-superheroes-superheroes.jpg",
//     homeworld: "Earth",
//     description:
//       "Steve Rogers, Captain America, is the embodiment of honor and virtue. Enhanced with peak human strength and wielding a Vibranium shield, he stands as a symbol of justice and resilience. Steve's unwavering commitment to the ideals of heroism makes him a cornerstone of the Avengers and the Marvel Universe.",
//   },
//   {
//     name: "Sam Wilson",
//     alias: "Falcon",
//     abilities: ["Wingsuit", "Telepathic link with birds"],
//     team: "Avengers",
//     img1: "https://i.pinimg.com/736x/0e/be/ed/0ebeed0c9d6578732a07ca98fee71e22.jpg",
//     homeworld: "Earth",
//     description:
//       "Sam Wilson, the Falcon, soars through the skies with his high-tech wingsuit and possesses a unique telepathic link with birds. As an Avenger, Sam exemplifies freedom, hope, and unity, fighting alongside his fellow heroes to maintain peace and protect the world from impending threats.",
//   },
// ];

// console.log(characters);


export const characters = [
  //Sukuna
  {
    name: "Ryomen Sukuna",
    alias: "King of Curses",
    img1: "https://i.pinimg.com/originals/9c/72/dc/9c72dccc3a57c0e6c80d66312cd4b938.jpg",
    homeworld: "Ryoku Sukuna",
    abilities: [
      "Superhuman strength",
      "Dismantle(cursed technique)",
      "Cleave(cursed technique)"
    ],
    team: "Akuyaku",
    description:
      "He is a legendary curse with a fearsome reputation, and his fingers, containing portions of his power. He currently hosts in Itadori Yuji. Despite his overwhelming power, Sukuna can be controlled to some extent by skilled jujutsu sorcerers using specific techniques.",
  },
  //Fushiguro Toji
  {
    name: "Fushiguro Toji",
    alias: "The invisible man",
    abilities: ["Enhanced senses", "martial arts", "Speed and reflexes"],
    team: "Akuyaku",
    img1: "https://i.pinimg.com/originals/59/8b/ba/598bba52c7167927a20448f071d16bd4.jpg",
    homeworld: "Fushiguro Toji",
    description:
      "Toji is from the Zenin Clan, but he was ostracized because he has no cursed energy. He often experiences chronic pain that feels like constant stabbing. Known for his ruthless approach to eliminating curses, he has a complicated relationship with his son, Megumi Fushiguro. Toji's main vulnerability lies in his lack of cursed energy.",
  },
  //Madara Uchiha
  {
    name: "Madara Uchiha",
    alias: "Ghost of the Uchiha",
    abilities: ["Wood Style Technique", "Sharingan", "Rinnegan"],
    team: "Akuyaku",
    img1: "https://i.pinimg.com/originals/7b/e1/2a/7be12a7230b6a41f3360e914ae136355.jpg",
    homeworld: "Madara Uchiha",

    description:
      " He is a legendary figure in the ninja world, the co-founder of the village of Konohagakure (the Village Hidden in the Leaves) and its first Rogue Ninja. He's hostile, aggressive, ambitious, merciless, vengeful but a prodigy. Madara's primary weakness lies in his vulnerability to sealing techniques, especially those targeting his eyes.",
  },
// Kira Yoshikage
  {
    name: "Kira Yoshikage",
    alias: " Kosaku Kawajiri",
    abilities: ["Time manipulation", "Killer QUeen Stand"],
    team: "Akuyaku",
    img1: "https://i.pinimg.com/originals/b7/47/b8/b747b808a713c318a56698f6ab306548.jpg",
    homeworld: "Kira Yoshikage",

    description:
      "He is a calm and methodical serial killer with a deadly Stand, characterized as highly egocentric and psychopathic, with a few obsessive-compulsive tendencies. He is also a hand fetishist. Despite his formidable Stand abilities, Kira's obsession with a quiet life and his desire for anonymity become significant weaknesses.",
  },

  // Sosuke Aizen
  {
    name: "Sosuke Aizen",
    alias: "Ruler of Hueco Mundo",
    abilities: ["Soul reaper", "Zanpakuto Kyoka Suigetsu(complete hypnosis)"],
    team: "Akuyaku",
    img1: "https://i.pinimg.com/originals/e1/1b/af/e11baf8fb45b7d95f13ea7024f3380c7.jpg",
    homeworld: "Sosuke Aizen",
    img3: "url_to_img3_for_Natasha_Stark",
    description:
      "Sosuke Aizen is a former captain of the Gotei 13 who orchestrates complex plots to achieve his goals and becomes a significant threat to the Soul Society. Sosuke Aizen is a soft spoken and very intelligent man, who hides a dark and treacherous personality beneath his very warm appearance. Aizen's power, Kyoka Suigetsu, has limitations, and its effectiveness can be countered with specific strategies.",
  },
  //Dio Brando
  {
    name: "Dio Brando",
    alias: "Shadow DIO",
    abilities: ["Vampire", "The World Stand(stops time)", "Regeneration"],
    team: "Akuyaku",
    img1: "https://i.pinimg.com/originals/b5/4a/c0/b54ac0a87f548df9db360cfb0764e750.jpg",
    homeworld: "Dio Brando",
    description:
      " Dio is ambitious, selfish, arrogant, callous and megalomaniacal, seemingly able to do anything to achieve his aims. His natural ruthlessness is exposed very early when plans to exploit the Joestar family despite them being kind enough to adopt him. He has a Stand called The World, which is a powerful astral projection that can stop time.  Dio's reliance on The World's time-stop ability becomes a crucial aspect of his vulnerability.",
  },
// Griffith
  {
    name: "Griffith",
    alias: " Blessed King of Longing",
    abilities: ["Reality Warping abilities", "Matter manipulation", "Crimson Beheilt"],
    team: " Akuyaku",
    img1: "https://i.pinimg.com/originals/45/a2/ca/45a2caf9da791432cab2383dce08e3e7.jpg",
    homeworld: "Griffith",
    description:
      "The leader of the mercenary group, the Band of the Hawk, Griffith is a handsome, charismatic, skilled, but self-serving and evil character who causes suffering. Griffith's actions and choices are influenced by complex emotions and desires, adding a layer of vulnerability.",
  },
  //Frieza
  {
    name: "Frieza",
    alias: "Lord Frieza",
    abilities: ["Super strength", "Ki sense", "Hell Bazooka among others"],
    team: " Akuyaku",
    img1: "https://i.pinimg.com/originals/13/42/5c/13425c130d866de9b1afc7dfe2ce3f54.jpg",
    homeworld: "Frieza",
    description:
      "Frieza is one of the strongest aliens in Dragon Ball Z. He enjoys invading other planets. Frieza is a very mean person who thinks he is very strong. Frieza is known for his ruthlessness and power. He is a mutant and a malignant narcissist. As the ruler of the Universe, he poses a significant threat to the Saiyan warriors and other inhabitants of the Dragon Ball world. Frieza's overconfidence and tendency to toy with opponents sometimes lead to his downfall. Additionally, his final form has certain durability limits. That's one too many weakness",
  },
];

console.log(characters);
